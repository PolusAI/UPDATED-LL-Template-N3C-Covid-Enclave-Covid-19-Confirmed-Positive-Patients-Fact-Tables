...
def COHORT(measurement, concept_set_members, person, location, manifest, condition_occurrence, microvisit_to_macrovisit_lds):
    ...
    #     .coalesce(1) \
    #     .dropDuplicates(["person_id", "macrovisit_start_date"]) #hospital - Modified by SM 20th Aug, removed orderBY/Coalesce. These cause problems and actually not needed
    hosp_visits = visits_df.where(F.col("macrovisit_start_date").isNotNull()) \
    ...
    df = df.select(
        ...
        'county',
        'age_at_covid',
        'date_of_birth', # Edited by SM Aug 20th. Important to include DOB in the long dataset if we change index date
        'race',
        'race_ethnicity',
        'data_partner_id',
