...
@transform_df(
    Output(""),
    microvisit_to_macrovisit_lds=Input(""),
    COHORT=Input(""),
    conditions_of_interest=Input(""),
    visits_of_interest=Input(""),
    procedures_of_interest=Input(""),
    observations_of_interest=Input(""),
    drugs_of_interest=Input(""),
    COVID_deaths=Input(""),
    devices_of_interest=Input(""),
    vaccines_of_interest=Input(""),
    quali_measurements_of_interest=Input(""),
    quant_measurements_of_interest=Input(""),
)
...
def cohort_all_facts_table(conditions_of_interest, visits_of_interest, procedures_of_interest, observations_of_interest, drugs_of_interest, COVID_deaths, COHORT, devices_of_interest, microvisit_to_macrovisit_lds, vaccines_of_interest, quant_measurements_of_interest, quali_measurements_of_interest):
    ...
    # macrovisits_df = microvisit_to_macrovisit_lds # Commented out by SM Aug 23
    ...
    # Edited by SM Aug 23- cleaned up outer joins
    visit_dates_df = microvisit_to_macrovisit_lds.select('person_id','visit_start_date').withColumnRenamed('visit_start_date','date')
    df = visit_dates_df
    
    join_keys = ['person_id','date']
    ...
    # EDITED SM AUG 23 2026 - Edited code above to use ONE select statement - SM Aug 23
    df = df.selectExpr('*', 
    'CASE WHEN DATEDIFF(COVID_first_poslab_or_diagnosis_date,date)>=0 THEN 1 ELSE 0 END AS pre_COVID',
    'CASE WHEN DATEDIFF(COVID_first_poslab_or_diagnosis_date,date)<0 THEN 1 ELSE 0 END AS post_COVID')
    ...
    macrovisits_df = microvisit_to_macrovisit_lds \
        ...
        .distinct()
    ### EDITED SM AUG 23 2026 - the original join below was very expensive and unnecessary (was an unnecessary cross join producing an explosion of rows)
    ### The code has been edited so that we only keep dates in DF (our main df, containing ALL visits) that occurred during a hospitalization (macrovisit)
    ### This will enable us to identify only hospital visits (macrovisits)
    ### The left_semi join will not keep any columns from the macrovisit table. Meaning macrovisit_start_date and end_date will not be in the final output; 
    # df_hosp = df.select('person_id', 'date').distinct().join(macrovisits_df, on='person_id', how= 'outer')
    df_hosp = df.select("person_id", "date").alias('d').join(macrovisits_df.alias('m'), on = (F.col('d.person_id')==F.col('m.person_id')) & (F.col('d.date').between(F.col('m.macrovisit_start_date'), F.col('m.macrovisit_end_date'))), how = 'left_semi').withColumn("during_macrovisit_hospitalization",F.lit(1))
    ### EDITED SM AUG 23 2026 - code commented out because the action is already being done above. We are trying to identify dates that occurred during a hospitalization (macrovisit) and then label them 1. 
    ...
    ### Now join the main df and hosp_df together, so we know which visit dates occurred during a macrovisit hospitalization
    df = df.join(df_hosp, on=['person_id','date'], how="left")
    # EDITED SM AUG 23 2026 - added code below Aug 23 2026 to fill in nulls on during_macrovisit_hospitalization
    df = df.fillna({"during_macrovisit_hospitalization": 0})
    
    # #fill of null non-continuous variables with 0 - EDITED SM AUG 23 2026 as original code logic may have not been correct
    ...
    df = df.na.fill(value=0, subset = [c.name for c in df.schema.fields if not isinstance(c.dataType,(DoubleType, LongType, DateType, StringType))])
