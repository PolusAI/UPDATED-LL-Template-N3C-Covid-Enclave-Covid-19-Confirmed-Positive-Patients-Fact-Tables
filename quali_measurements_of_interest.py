...
def quali_measurements_of_interest(measurement, concept_set_members, COHORT, customize_concept_sets):
    ...
    #Modified Code above based on new structure of fusion sheet. Creates a subsetted fusion sheet to the qual measures. 
    fusion_df = customize_concept_sets \
        .filter(customize_concept_sets.domain.contains('measurement')) \
        .filter(customize_concept_sets["is_qual"] == 1) \
        .select('concept_set_name', 'codeset_id', 'indicator_prefix', 'lower_bound', 'upper_bound')
    
    # Takes the concept_set_members table
    ...
    #use only concept_set_name. Join the concept_set_members to our subsetted fusion sheet. 
    else:
    ...
    for indicator in df.select("indicator_prefix").distinct().collect():
        ...
        df = df.withColumn(pos_column, F.when(df["value_as_concept_id"].isin(positive_measurement_ids), 1).otherwise(0))
    
        # Create _Neg columns: 1 if value_as_concept_id exists in negative_measurement_ids, 0 otherwise
        df = df.withColumn(neg_column, F.when(df["value_as_concept_id"].isin(negative_measurement_ids), 1).otherwise(0))
    ...
    df = df.groupBy("person_id", "date").agg(*[aggregations[col] for col in columns_to_agg])
